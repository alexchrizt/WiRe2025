...
# --- Aufgabe 4.2a ----------------------------------------------------------
    ...
    # (Bitte in beobachtungen.txt übernehmen)
    ...
    # (Verhalten und Ursache bitte in beobachtungen.txt notieren)
    ...
    for t in t_vals:
        ...
        # 1) Frobenius-Norm von A
        normA   = frobenius_norm(A)

        # 2) Inverse per Hand für 2×2-Matrix:
        #    det = 2*(2*10^t) - 1*1 = 4*10^t - 1
        det = 4 * 10**t - 1
        invA = np.array([[ 2 * 10**t, -1],
                         [       -1,  2]], float) / det
